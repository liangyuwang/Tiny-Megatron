...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        tp_size = 3 * C // qkv.size(-1) # for tensor parallel
        hidden_size_per_partition = self.n_embd // tp_size
        heads_per_partition = self.n_head // tp_size
        q, k, v = qkv.chunk(3, dim=-1)
        hs = hidden_size_per_partition // heads_per_partition
        q = q.view(B, T, heads_per_partition, hs)
        k = k.view(B, T, heads_per_partition, hs)
        v = v.view(B, T, heads_per_partition, hs)
        ...
        y = y.view(B, T, hidden_size_per_partition)
        y = self.c_proj(y)
        return y
